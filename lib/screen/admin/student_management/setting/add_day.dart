...
//   final String? status;
//   final int? id;
//   const AddDay({super.key, this.status, this.name, this.id});
//
//   @override
...
class AddDay extends StatefulWidget {
  ...
  final String? status;
  final int? id;
  const AddDay({Key? key, this.status, this.name, this.id}) : super(key: key);
...
class _AddDayState extends State<AddDay> {
  ...
  void initState() {
    nameController.text = widget.name ?? '';
    widget.status != null && widget.status == 'Active'
        ? valueData = true
        : valueData = false;
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return BuildAppBar(
      centerTitle: true,
      title: widget.name == null ? 'add_day' : 'edit_day',
      bodyWidget: Padding(
        padding: const EdgeInsets.all(8.0),
        child: Column(
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: BuildButton(
                    text: widget.name == null ? 'submit' : 'save',
                    function: () {
                      Day updatedData = Day(
                      ...
                      if (widget.name == null) {
                        dayController.createDay(
                            nameController.text, status, context);
                        Navigator.pop(context, updatedData);
                      } else {
                        dayController.updateDay(
                            widget.id!, nameController.text, status, context);
                        Navigator.pop(context, updatedData);
                      }
                  ...
                  Expanded(
                    child: BuildButton(
                      ...
                      function: () async {
                        ...
                        Navigator.pop(context, updatedData); // Pass updatedData back
                      },
                    ),
                  ),
              ],
